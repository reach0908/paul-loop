...
`ac-verify.sh` resolves `artifacts:` from its physical invocation directory; run it from the
intended worktree root. The plan file's location does not change this boundary. Standalone,
non-Git directories remain supported. Paths are comma-separated; absolute paths and any `..`
segment are rejected, including absolute paths that happen to point inside the worktree.

Each declared artifact must be a regular file or directory inside that boundary. Symlinks within
the boundary are allowed; external or dangling links fail. Without `verify:`, `expect:` searches
literal bytes in files and recursively in directories, validating every traversed path even after
a match. Internal directory cycles are visited once. With `verify:`, only the command log remains
the `expect:` corpus; artifacts still undergo their independent existence/boundary check.
Empty comma-only artifact lists fail. Missing artifacts, invalid paths and read errors fail the
AC and its aggregate verdict/state. Files generated by `verify:` are checked after it finishes.

File reads use the same no-follow descriptor that supplies the regular-file check. This rejects
a final-file symlink substitution before open. It is not a sandbox for `verify:` commands, a
guarantee against hard links, or isolation from same-user processes concurrently replacing ancestor
directories. One file is buffered at a time for literal matching; large generated files may need
