...
- **"Does this logic / state model feel right?"** → [LOGIC.md](LOGIC.md). Default to a tiny interactive terminal app; use a self-contained HTML demo when non-developers need to drive it or the review needs a shareable file.
...
2. **Trivial to run.** Use one command from the project's existing task runner — `pnpm <name>`, `python <path>`, `bun <path>`, etc. A self-contained HTML demo opens directly in a browser without a server or installation.
