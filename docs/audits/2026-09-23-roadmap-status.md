...
소비 프로젝트의 설치·활성화나 실제 속도·기억 효용까지 확인됐다는 의미는 아니다.

현재 작업은 [CI 중복 제거와 #101](2026-09-23-ci-simplification.md)이다.
다음은 실제 확인한 engine의 `origin/develop` 기본값 개선과 #96의 시크릿 검사 정책 보완,
같은 조건의 native 과제 4쌍 비교,
실제 반복 실패 6–10건의 기억 효용 평가 순서다. 폴루프 자체 개선은 이 관측에 따라
불필요한 절차를 줄이고 실제 실패 경로를 고치는 방향이며 추가 메모리 인프라가 기본값은 아니다.
아래는 이전 관측 시점의 이력으로 보존하며, 당시 후보/다음 작업을 현재 미완료로 중복 처리하지 않는다.
...
| 3 | native 선행 조건 통제 후 같은 조건의 과제 4쌍 비교 | [#111 후속 관측](2026-09-23-role-dispatch.md): 권한 불일치는 역할 작업 전 BLOCK, 부모부터 read-only인 정상/결함 계획은 각각 PASS/BLOCK. 넓은 부모 아래 자식만 좁히는 권한 격리와 변동하는 자식 카탈로그는 미해결. 아직 속도 개선율 없음 |
| 4 | 실제 반복 실패 6–10건의 file/native memory 효용 비교 | [기본 저장소 표본 점검](2026-09-23-role-dispatch.md#메모리-평가-표본-점검): 지정한 3개 저장소의 등록 worktree 50개에서 기본 lesson/shared history JSON 0건. 사용자 지정 경로·DB·native memory 전체의 미사용 판정은 아님. 실제 검증 receipt 확보 후 기록→검색→관련성→활용→독립 검증 비교 |
